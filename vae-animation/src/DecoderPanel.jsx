    ...
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold mb-2">
          Decoder: <span className="text-orange-400">From Latent to Data</span>
        </h2>
      ...
      <div className="flex justify-center gap-4">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition-all ${
            !generationMode ? 'bg-orange-600 text-white' : 'bg-white/10 text-gray-400'
          }`}
        >
          Reconstruction Mode
        </button>
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${
            generationMode ? 'bg-purple-600 text-white' : 'bg-white/10 text-gray-400'
          }`}
        >
          <Sparkles size={18} />
          Generation Mode
      ...
      <div className={`rounded-xl p-4 border ${generationMode ? 'bg-purple-900/20 border-purple-500/30' : 'bg-orange-900/20 border-orange-500/30'}`}>
        {generationMode ? (
          <p className="text-gray-300">
            <strong className="text-purple-400">Generation Mode:</strong> Sample z ~ N(0, I) directly (no encoder needed!) 
            and pass through decoder to generate new data that looks like the training distribution.
          </p>
        ) : (
          <p className="text-gray-300">
            <strong className="text-orange-400">Reconstruction Mode:</strong> The encoder produces z from input x, 
      ...
      <div className="bg-black/30 rounded-2xl p-8 border border-white/10">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className={`transition-all duration-500 ${steps[currentStep].highlight === 'latent' ? 'scale-110' : ''}`}>
            ...
            <p className="text-center text-xs text-gray-500">[batch, 20]</p>
          </div>
          ...
          <div className={`transition-all duration-500 ${steps[currentStep].highlight === 'image' ? 'scale-110' : ''}`}>
            ...
            <p className="text-center text-xs text-gray-500">28×28</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/40 rounded-xl p-4 border border-white/10">
        <p className="text-sm text-gray-400 mb-3">PyTorch Decoder Implementation:</p>
      ...
      <div className="bg-black/30 rounded-xl p-4 border border-white/10">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="bg-white/5 rounded-lg p-3">
            <h5 className="font-medium text-orange-400">MLP Decoder</h5>
            <p className="text-xs text-gray-400 mt-1">Fully connected layers. Simple but works for small images like MNIST.</p>
          </div>
          <div className="bg-white/5 rounded-lg p-3">
            <h5 className="font-medium text-orange-400">Transposed Conv (Deconv)</h5>
            <p className="text-xs text-gray-400 mt-1">ConvTranspose2d layers for upsampling. Better for larger images.</p>
          </div>
          <div className="bg-white/5 rounded-lg p-3">
            <h5 className="font-medium text-orange-400">Upsample + Conv</h5>
            <p className="text-xs text-gray-400 mt-1">Nearest/bilinear upsample followed by conv. Avoids checkerboard artifacts.</p>
          </div>
        </div>
      </div>
    </div>
