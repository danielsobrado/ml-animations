import{j as e}from"./react-vendor-Cdu38Wyn.js";import{C as t}from"./CodeFixLab-Cb3BWYdJ.js";import"./icons-C7miCxLM.js";const n=[{id:"dot-product-first-pair",stepLabel:"1.1",group:"Dot product",title:"First matching pair",concept:"A dot product starts by multiplying entries with the same index.",objective:"Replace one number with the first pair product.",difficulty:"warmup",starterCode:`function firstPairProduct(a, b) {
...
}`,explanation:"This is the dense-layer weight-gradient formula used in neural-network training."}],a=[{id:"transformer-token-embedding-lookup",stepLabel:"41.1",group:"Transformer mini-block shapes",title:"Token embedding lookup",concept:"A token ID selects one row from the embedding table.",objective:"Return embeddingTable[tokenId].",difficulty:"warmup",starterCode:`function lookupEmbedding(embeddingTable, tokenId) {
...
}`,explanation:"Many transformer bugs are shape bugs: Q and K must line up for similarity scores."}],o=[{id:"lm-vocab-size",stepLabel:"48.1",group:"Mini vocabulary and logits",title:"Vocabulary size",concept:"A language model predicts one score per vocabulary token.",objective:"Return the number of tokens in the vocabulary.",difficulty:"warmup",starterCode:`function vocabSize(vocab) {
...
return results;`,hints:["The chunks are already sorted by relevance.","Use the same budget check as greedy packing.","If it fits, push chunk.id and update used.",`if (used + chunk.tokens <= budget) {
  ...
  return packed;
}`,explanation:"Prompt packing balances relevance against token budget. The best chunk is not useful if it crowds out required evidence."}],s=[{id:"eval-true-positive",stepLabel:"62.1",group:"Confusion matrix",title:"True positive",concept:"A true positive happens when the model predicts positive and the true label is positive.",objective:"Return true only when prediction and label are both 1.",difficulty:"warmup",starterCode:`function isTruePositive(prediction, label) {
...
return results;`,hints:["Use adjustmentSet.includes(confounders[i]).","If one is missing, return false.","if (!adjustmentSet.includes(confounders[i])) return false;"],solution:`function coversConfounders(adjustmentSet, confounders) {
  ...
  return true;
}`,explanation:"This toy check is not full d-separation, but it reinforces the adjustment-set idea."}],u=[...n,...r,...a,...o,...i,...s,...c];function h(){return e.jsx(t,{exercises:u})}export{h as default};
