...
}`,explanation:"Bias correction keeps Adam from underestimating early moments, while the second moment still rescales coordinates with different gradient magnitudes."}]),M=r("advanced-models",{kind:"advanced-model pipeline",signalName:"retrieval or multimodal score",stages:["encode","retrieve","ground"],stageExplanation:"Advanced model systems often encode inputs, retrieve or combine evidence, and check grounding."}),P=r("math-fundamentals",{kind:"mathematical computation",signalName:"numeric fit or stability score",stages:["represent","compute","check"],stageExplanation:"Math code is clearer when representation, computation, and result checks are separate."}),_=r("core-ml",{kind:"machine-learning workflow",signalName:"validation metric",stages:["split","train","evaluate"],stageExplanation:"Core ML workflows depend on clean data splits, training logic, and honest evaluation."}),T=r("model-reliability",{kind:"reliability check",signalName:"monitoring or risk score",stages:["observe","alert","triage"],stageExplanation:"Reliability systems observe behavior, alert on meaningful shifts, and triage failures."}),z=r("experimentation-causal-ml",{kind:"experiment analysis",signalName:"effect or balance score",stages:["assign","measure","compare"],stageExplanation:"Experiment code must separate assignment, measurement, and comparison to support causal claims."}),j=r("probability-stats",{kind:"probability/statistics calculation",signalName:"probability or uncertainty score",stages:["count","normalize","summarize"],stageExplanation:"Probability code often counts outcomes, normalizes them, then summarizes uncertainty."}),B=r("reinforcement-learning",{kind:"reinforcement-learning loop",signalName:"return or action-value score",stages:["observe","act","learn"],stageExplanation:"RL loops observe state, choose actions, and update behavior from feedback."}),D=h(B,"ppo-clipped-policy-gradient",()=>[{id:"ppo-policy-ratio",title:"Compute the policy ratio",concept:"PPO compares the new policy probability with the old collection-policy probability for the sampled action.",objective:"Return pi_new divided by pi_old.",difficulty:"core",starterCode:`function policyRatio(newProbability, oldProbability) {
...
return results;`,hints:["PPO selects Math.min(unclipped, clipped).","A row is clipped when the selected value equals clipped and differs from unclipped.","if (Math.min(unclipped, clipped) !== unclipped) clippedCount += 1;"],solution:`function countClippedRows(rows, epsilon) {
  ...
  return clippedCount;
}`,explanation:"The minibatch audit links the PPO formula to the lesson table: not every out-of-band ratio clips, because the advantage sign decides which side is conservative."}]),I=r("algorithms",{kind:"algorithmic data structure",signalName:"rank or membership score",stages:["insert","query","verify"],stageExplanation:"Algorithmic structures are useful when updates, queries, and checks are kept explicit."}),q=r("diffusion-models",{kind:"diffusion-model pipeline",signalName:"noise or denoising score",stages:["noise","condition","denoise"],stageExplanation:"Diffusion systems manage noise, conditioning, and denoising as separate implementation stages."}),g=[...C,...$,...R,...A,...M,...P,..._,...T,...z,...j,...D,...I,...q],J=Object.fromEntries(g.map(e=>[e.lessonId,e]));g.flatMap(e=>e.exercises);function V(e){return J[e]||null}function U(e){var a;return((a=V(e))==null?void 0:a.exercises)||[]}export{g as L,U as g};
