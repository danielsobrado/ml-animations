  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-emerald-400 mb-2">T5 Model Scaling</h2>
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {Object.entries(models).map(([key, model]) => (
          ...
          >
            {model.name}
            {model.sdUse && <span className="ml-2 text-yellow-400 text-xs">★ SD3</span>}
          </button>
        ))}
      </div>
      ...
      <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 rounded-xl p-6 border border-emerald-500/30">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-emerald-400">{selectedModelData.name}</h3>
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-6">
          <div className="bg-black/30 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-emerald-400">{selectedModelData.params}</div>
            <div className="text-sm text-gray-400">Parameters</div>
          </div>
          <div className="bg-black/30 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-teal-400">{selectedModelData.layers}</div>
            <div className="text-sm text-gray-400">Encoder Layers</div>
          </div>
          <div className="bg-black/30 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-cyan-400">{selectedModelData.hidden}</div>
            <div className="text-sm text-gray-400">Hidden Dim</div>
          </div>
          <div className="bg-black/30 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-blue-400">{selectedModelData.heads}</div>
      ...
      <div className="bg-black/40 rounded-xl p-6">
        <h3 className="font-semibold text-gray-300 mb-4">📊 Parameter Scale Comparison</h3>
          ...
          {Object.entries(models).map(([key, model]) => {
            ...
            return (
              <div key={key} className="flex items-center gap-4">
                <div className="w-20 text-sm text-gray-400">{model.name}</div>
      ...
      <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl p-6 border border-purple-500/30">
        <h3 className="font-semibold text-purple-400 mb-4">📈 Scaling Laws in Practice</h3>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b border-white/5">
                  <td className="py-2 px-3 text-gray-400">Params</td>
                  <td className="py-2 px-3 text-emerald-400">3.5×</td>
                  <td className="py-2 px-3 text-emerald-400">4×</td>
                  <td className="py-2 px-3 text-emerald-400">3.7×</td>
                </tr>
                <tr className="border-b border-white/5">
                  <td className="py-2 px-3 text-gray-400">Hidden Dim</td>
                  <td className="py-2 px-3">768→1024</td>
                  <td className="py-2 px-3">1024→2048</td>
                  <td className="py-2 px-3">2048→4096</td>
                </tr>
                <tr className="border-b border-white/5">
                  <td className="py-2 px-3 text-gray-400">Memory</td>
      ...
      <div className="bg-emerald-500/10 rounded-xl p-6 border border-emerald-500/30">
        <h3 className="font-semibold text-emerald-400 mb-3">💡 Practical Considerations</h3>
        <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-300">
          <div className="bg-black/30 rounded-lg p-4">
