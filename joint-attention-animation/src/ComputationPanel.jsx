    ...
    <div className="space-y-8">
      ...
      <div className="bg-black/30 rounded-2xl p-6 border border-white/10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-violet-900/30 rounded-xl p-4 border border-violet-500/30">
            <p className="text-xs text-gray-400 mb-1">Total Sequence</p>
            <p className="text-2xl font-bold text-violet-400">{formatNumber(totalTokens)}</p>
            <p className="text-xs text-gray-500">{imgTokens} img + {txtLen} txt</p>
          </div>
          <div className="bg-blue-900/30 rounded-xl p-4 border border-blue-500/30">
            <p className="text-xs text-gray-400 mb-1">Attention FLOPs</p>
            <p className="text-2xl font-bold text-blue-400">{formatNumber(attentionFlops)}</p>
            <p className="text-xs text-gray-500">per layer</p>
          </div>
          <div className="bg-orange-900/30 rounded-xl p-4 border border-orange-500/30">
            <p className="text-xs text-gray-400 mb-1">Attn Matrix Memory</p>
            <p className="text-2xl font-bold text-orange-400">{formatBytes(attentionMemory)}</p>
            <p className="text-xs text-gray-500">FP16, per layer</p>
          </div>
          <div className="bg-green-900/30 rounded-xl p-4 border border-green-500/30">
            <p className="text-xs text-gray-400 mb-1">vs Cross-Attn</p>
            <p className="text-2xl font-bold text-green-400">
              {((attentionFlops / crossAttnFlops - 1) * 100).toFixed(0)}%
            </p>
            <p className="text-xs text-gray-500">more compute</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/30 rounded-2xl p-6 border border-white/10">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-900/20 rounded-xl p-4 border border-blue-500/30">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-bold text-blue-400">Cross-Attention (for comparison)</h4>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/20">
                ...
                <th className="py-3 px-4 text-left text-gray-400">Image Tokens</th>
                <th className="py-3 px-4 text-left text-gray-400">Total Seq</th>
                <th className="py-3 px-4 text-left text-gray-400">Attn Memory</th>
                <th className="py-3 px-4 text-left text-gray-400">Total VRAM</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/10">
              ...
              <tr>
                ...
                <td className="py-3 px-4 text-orange-400">~12 GB</td>
              </tr>
              <tr>
      ...
      <div className="bg-black/30 rounded-2xl p-6 border border-white/10">
        ...
        <div className="bg-black/50 rounded-lg p-4 font-mono text-sm overflow-x-auto">
          <pre className="text-gray-300">{`# In diffusers, enable memory-efficient attention
