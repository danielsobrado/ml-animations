...
test('hypothesis testing intuition has a complete curated 100-question assessment', () => {
  ...
  for (const [index, question] of quiz.entries()) {
    assert.ok(question.id.startsWith('ht-'), `question ${index + 1} should use the ht id prefix`);
...
test('hypothesis testing intuition assessment covers learning points in the right order', () => {
  ...
  const orderedMilestones = [
    ['no-effect baseline purpose', ['no-effect baseline']],
    ['null hypothesis', ['null hypothesis']],
    ['observed effect', ['empirical signal']],
    ['standard error', ['standard error']],
    ['test statistic', ['test statistic']],
    ['null distribution', ['null distribution']],
    ['p-value definition', ['tail probability']],
    ['alpha threshold', ['alpha']],
    ['type i error', ['type i error']],
    ['power', ['power']],
    ['minimum detectable effect', ['minimum detectable effect']],
    ['confidence interval connection', ['confidence intervals connect']],
    ['randomization and confounding', ['confounding']],
    ['multiple testing caution', ['multiple comparisons']],
    ['scenario application', ['a/b test']],
...
test('hypothesis testing intuition assessment avoids unsafe misconception keying', () => {
  ...
  const unsafePatterns = [
    /probability that the null hypothesis is true/i,
    /proven true/i,
    /proves/i,
    /guarantee/i,
    /guarantees/i,
    /with certainty/i,
    /automatically important/i,
    /automatically large enough/i,
    /automatically valuable/i,
    /assumption-free/i,
    /always preserves/i,
    /always worth/i,
  ];
  ...
  for (const [index, question] of quiz.entries()) {
    ...
    const explicitTrapPrompt = /false|misleading|unsafe|too strong|wrong|failure mode|correct/i.test(question.prompt);
