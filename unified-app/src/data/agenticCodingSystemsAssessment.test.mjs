...
test('agentic coding systems assessment avoids duplicate prompts and correct answers', () => {
  const prompts = AGENTIC_CODING_SYSTEMS_QUIZ.map((item) => normalize(item.prompt));
  const correctAnswers = AGENTIC_CODING_SYSTEMS_QUIZ.map((item) => normalize(correctAnswer(item)));
