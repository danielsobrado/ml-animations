...
test('omni multimodal architectures assessment avoids duplicate prompts and correct answers', () => {
  const prompts = OMNI_MULTIMODAL_ARCHITECTURES_QUIZ.map((item) => normalize(item.prompt));
  const correctAnswers = OMNI_MULTIMODAL_ARCHITECTURES_QUIZ.map((item) => normalize(correctAnswer(item)));
