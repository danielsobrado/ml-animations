...
test('unet vs dit has a complete curated 100-question assessment with focused labs', () => {
  const { quiz, labs } = getLessonAssessment('unet-vs-dit');
  ...
  for (const [index, question] of quiz.entries()) {
    assert.match(question.id, /^unetdit-\d{3}-[a-z0-9-]+$/);
