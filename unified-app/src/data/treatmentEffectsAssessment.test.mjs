...
test('treatment effects has a complete curated 100-question assessment', () => {
  ...
  for (const [index, question] of quiz.entries()) {
    assert.ok(question.id.startsWith('te-'), `question ${index + 1} should use the te id prefix`);
...
test('treatment effects assessment covers learning points in the right order', () => {
  ...
  const orderedMilestones = [
    ['purpose', ['averages', 'hide']],
    ['potential outcomes', ['potential outcome']],
    ['ATE', ['ate', 'average']],
    ['CATE', ['cate', 'segment']],
    ['uplift', ['uplift', 'incremental']],
    ['heterogeneity', ['heterogeneity']],
    ['segment share', ['segment share']],
    ['targeting', ['targeting']],
    ['weighted average', ['population shares']],
    ['subgroup precision', ['subgroup', 'noisier']],
    ['predefined segments', ['predefine']],
    ['policy value', ['policy value']],
    ['scenario application', ['returning users', 'harmed']],
...
test('treatment effects assessment avoids unsafe misconception keying', () => {
  ...
  const unsafePatterns = [
    /every segment benefits/i,
    /just the treated response rate/i,
    /always target the subgroup/i,
    /unlimited subgroup mining safe/i,
    /regardless of cost/i,
    /highest baseline outcome because they have highest uplift/i,
    /automatically confirmatory/i,
    /without uncertainty/i,
    /always enough for rollout/i,
    /with no comparable controls/i,
    /always a safe targeting segment/i,
    /always transfers unchanged/i,
    /ignore harmed important groups/i,
    /just the largest CATE/i,
    /one noisy subgroup win proves/i,
  ];
