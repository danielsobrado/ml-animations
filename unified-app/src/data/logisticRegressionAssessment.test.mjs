...
test('logistic regression has a complete curated 100-question assessment', () => {
  ...
  for (const [index, question] of quiz.entries()) {
    assert.ok(question.id.startsWith('logreg-'), `question ${index + 1} should use the logreg id prefix`);
...
test('logistic regression assessment covers learning points in the right order', () => {
  ...
  const orderedMilestones = [
    ['purpose', ['binary class']],
    ['linear logit', ['linear score', 'logit']],
    ['sigmoid', ['maps any real-valued score']],
    ['threshold', ['classification threshold']],
    ['binary cross-entropy', ['binary cross-entropy']],
    ['calibration caveat', ['not automatically well calibrated']],
    ['logit formula', ['w dot x plus b']],
    ['log-odds', ['log-odds']],
    ['regularization', ['regularization']],
    ['validation', ['validation']],
    ['scenario application', ['score 0.62']],
...
test('logistic regression assessment avoids unsafe misconception keying', () => {
  ...
  const unsafePatterns = [
    /unbounded continuous value as the final output/i,
    /only 0 or 1 exactly/i,
    /retrains all model weights automatically/i,
    /always learns a nonlinear boundary/i,
    /automatically calibrated/i,
    /guarantees the true class is positive/i,
    /automatically prove causal effects/i,
    /removes the need for validation/i,
    /never matters for regularized logistic regression/i,
    /small stable unregularized weights/i,
    /always exactly 0\.5/i,
    /ignores the observed label/i,
    /already a valid probability before sigmoid/i,
    /never affects threshold or calibration/i,
    /without scores or threshold/i,
  ];
...
test('logistic regression assessment distributes correct-answer positions across every page', () => {
  const { quiz } = getLessonAssessment('logistic-regression');
