...
test('rag assessment has 100 curated questions', () => {
  const { quiz, labs } = getLessonAssessment('rag');
  ...
  for (const [index, item] of quiz.entries()) {
    assert.match(item.id, /^rag-\d{3}-[a-z0-9-]+$/);
    assert.equal(item.id.startsWith(`rag-${String(index + 1).padStart(3, '0')}`), true, `${item.id} is out of order`);
...
test('rag assessment avoids duplicate prompts and correct answers', () => {
  const { quiz } = getLessonAssessment('rag');
...
test('rag assessment progresses through lesson learning points', () => {
  const { quiz } = getLessonAssessment('rag');
  const ranges = [
    ['Foundation', 0, 20],
    ['Mechanism', 20, 50],
  ...
  const milestones = [
    ['purpose', ['main purpose of RAG']],
    ['three steps', ['three high-level steps']],
    ['chunking', ['What is chunking']],
    ['embeddings', ['Why are embeddings used']],
    ['top-k', ['What does top-k retrieval return']],
    ['indexing order', ['indexing pipeline order']],
    ['runtime order', ['runtime RAG order']],
    ['grounding', ['grounding']],
    ['sandbox', ['sandbox']],
    ['misconception traps', ['Which statement is false about RAG']],
...
test('rag keeps misconception traps after setup', () => {
  const { quiz } = getLessonAssessment('rag');
  const misconceptionTerms = [
    /retrains the LLM on new data for every user query/i,
    /automatically grounded/i,
    /always improves answer quality/i,
    /proves the cited source is current and correct/i,
    /Chunk size is irrelevant/i,
    /represents every production RAG retriever/i,
    /automatically makes stale documents current/i,
    /permissions unnecessary/i,
    /fluent response means retrieval succeeded/i,
    /solve exactly the same problem/i,
    /can never need re-embedding or deletion/i,
    /Final answer accuracy alone/i,
    /silently blended/i,
    /Adding all retrieved documents is always better/i,
...
test('rag assessment avoids visible-page answer leakage', () => {
  const { quiz } = getLessonAssessment('rag');
  const pageSize = 10;
...
test('rag assessment distributes correct answer positions per page', () => {
  const { quiz } = getLessonAssessment('rag');
  const pageSize = 10;
