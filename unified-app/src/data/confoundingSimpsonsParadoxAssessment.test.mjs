...
test('confounding simpsons paradox has a complete curated 100-question assessment', () => {
  ...
  for (const [index, question] of quiz.entries()) {
    assert.ok(question.id.startsWith('conf-'), `question ${index + 1} should use the conf id prefix`);
...
test('confounding simpsons paradox assessment covers learning points in the right order', () => {
  ...
  const orderedMilestones = [
    ['observational comparison', ['observational comparison']],
    ['confounder', ['what is a confounder']],
    ['segment mix', ['segment mix']],
    ['aggregate effect', ['aggregate effect']],
    ['within segment effect', ['within-segment effect']],
    ['simpson paradox', ['aggregate comparison points differently']],
    ['like with like', ['compare like with like']],
    ['first check', ['first check']],
    ['weighted average', ['weighted average']],
    ['backdoor path', ['backdoor path']],
    ['overlap', ['overlap']],
    ['protocol', ['confounding analysis protocol']],
    ['scenario application', ['treatment has many more high-risk users']],
    ['tricky false claims', ['aggregate claim is false']],
...
test('confounding simpsons paradox assessment avoids unsafe misconception keying', () => {
  ...
  const unsafePatterns = [
    /guarantee/i,
    /guarantees/i,
    /always the best causal/i,
    /caused by the treatment after exposure/i,
    /automatically proves/i,
    /randomization is unnecessary/i,
    /adding every available variable always/i,
    /always correct for total effects/i,
    /always removes bias/i,
    /extrapolate perfectly/i,
    /proves no unmeasured/i,
    /fully removes confounding/i,
    /large association is enough/i,
    /proves the exact causal effect/i,
  ];
