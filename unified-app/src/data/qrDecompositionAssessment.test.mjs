...
test('qr decomposition assessment covers learning points in the right order', () => {
  ...
  assert.match(sections[0], /a = qr|q\^t q = i|orthonormal|upper triangular/);
  assert.match(sections[0], /gram-schmidt|least squares|normal equations/);
  assert.match(sections[1], /projection|r_ij|q\^t b|householder|pivoted|rank|condition/);
  assert.match(sections[1], /reconstruction|orthogonality|triangular|back substitution/);
  assert.match(sections[2], /scenario|production|numpy|residual|rank|monitor/);
  assert.match(sections[3], /trap|interview|misconception|stability|rank/);
});
...
test('qr decomposition assessment avoids unsafe misconception keying', () => {
  ...
  for (const [index, question] of quiz.entries()) {
    ...
    const explicitTrapPrompt = /trap|misconception|what is wrong|risky|avoid|interview|contrast/.test(prompt);
