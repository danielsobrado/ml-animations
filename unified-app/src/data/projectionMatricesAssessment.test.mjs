...
test('projection matrices assessment has 100 curated questions', () => {
  const { quiz, labs } = getLessonAssessment('projection-matrices');
  ...
  for (const [index, item] of quiz.entries()) {
    assert.match(item.id, /^pmat-\d{3}-[a-z0-9-]+$/);
    assert.equal(item.id.startsWith(`pmat-${String(index + 1).padStart(3, '0')}`), true, `${item.id} is out of order`);
...
test('projection matrices assessment avoids duplicate prompts and correct answers', () => {
...
test('projection matrices assessment progresses through lesson learning points', () => {
  const { quiz } = getLessonAssessment('projection-matrices');
  const ranges = [
    ['Foundation', 0, 20],
    ['Mechanism', 20, 50],
  ...
  const milestones = [
    ['purpose', ['main purpose of a projection matrix']],
    ['target subspace', ['target subspace']],
    ['idempotence rule', ['P^2 = P']],
    ['practice lab', ['Practice Lab ask']],
    ['code outer product', ['outer product']],
    ['mechanism', ['Why does Px lie on the teal line']],
    ['application', ['[[1,0],[0,0]] times [3,4]']],
    ['misconception traps', ['Which projection-matrix claim is false']],
...
test('projection matrices keeps misconception traps after setup', () => {
  ...
  const misconceptionTerms = [
    ...
    /move Px farther/i,
    /every square matrix as a projection matrix/i,
    /residual x - Px must lie inside/i,
    /Idempotence alone proves/i,
    /works unchanged even when u is not unit/i,
    /projects every vector to the x-axis/i,
    /Every projection residual must be zero/i,
    /Projection never depends on the metric/i,
    /A\(A\^T A\)\^-1 A\^T even when A\^T A is singular/i,
    /Px always equals the original x/i,
    /without specifying the target subspace/i,
    /entries are built by adding/i,
    /sorting v instead of multiplying/i,
...
test('projection matrices assessment avoids visible-page answer leakage', () => {
  const { quiz } = getLessonAssessment('projection-matrices');
  const pageSize = 10;
...
test('projection matrices assessment distributes correct answer positions per page', () => {
  const { quiz } = getLessonAssessment('projection-matrices');
  const pageSize = 10;
