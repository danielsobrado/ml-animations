...
test('fasttext assessment has 100 production-ready questions', () => {
  assert.equal(FASTTEXT_QUIZ.length, 100);
  const ids = new Set();
  const prompts = new Set();
  const correctAnswers = new Set();

  for (const [index, item] of FASTTEXT_QUIZ.entries()) {
    assert.match(item.id, /^ftx-\d{3}$/);
    assert.equal(ids.has(item.id), false, `duplicate id ${item.id}`);
    ids.add(item.id);
    assert.equal(item.id, `ftx-${String(index + 1).padStart(3, '0')}`);
...
test('fasttext assessment progresses from subword basics to deployment judgment', () => {
  const ranges = [
    ['Foundation', 0, 20],
    ['Mechanism', 20, 50],
    ['Application', 50, 75],
  ...
  for (const [level, start, end] of ranges) {
    assert.equal(FASTTEXT_QUIZ.slice(start, end).every((item) => item.level === level), true, `${level} range mismatch`);
  }

  const milestones = [
    ['subword premise', ['main idea behind FastText', 'character n-grams']],
    ['composition mechanism', ['word vector composed', 'summing the whole-word vector']],
    ['oov mechanism', ['kingship if unseen', 'extracts character n-grams']],
    ['application tradeoff', ['application-level FastText takeaway', 'resource cost']],
    ['trap setup', ['FastText is just Word2Vec with a new name', 'false']],
    ['interview synthesis', ['interview-ready FastText mastery', 'OOV robustness']],
  ];

  let previous = -1;
  for (const [name, terms] of milestones) {
    const index = FASTTEXT_QUIZ.findIndex((item) => (
...
test('fasttext assessment marks misconceptions as traps after setup', () => {
  const misconceptionTerms = [
    /just Word2Vec with a new name/i,
    /does not need any learned subword vectors/i,
    /always beats contextual models/i,
    /always uses less memory/i,
    /all OOV vectors are reliable/i,
    /hand-written linguistic rules/i,
    /boundary markers do nothing/i,
    /requires no labeled examples/i,
    /enough for launch/i,
    /optimal for every language/i,
    /fully understands spelling/i,
    /always mean similar product semantics/i,
    /remove preprocessing concerns/i,
    /handle OOV words the same way/i,
    /eliminates bias/i,
  ];
...
test('fasttext assessment avoids visible-page answer leakage', () => {
  const pageSize = 10;
  for (let pageStart = 0; pageStart < FASTTEXT_QUIZ.length; pageStart += pageSize) {
    const page = FASTTEXT_QUIZ.slice(pageStart, pageStart + pageSize);
...
test('fasttext assessment distributes correct answer positions per page', () => {
  const pageSize = 10;
  for (let pageStart = 0; pageStart < FASTTEXT_QUIZ.length; pageStart += pageSize) {
    const page = FASTTEXT_QUIZ.slice(pageStart, pageStart + pageSize);
    const counts = [0, 0, 0];
