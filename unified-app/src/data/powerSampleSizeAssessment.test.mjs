...
test('power sample size has a complete curated 100-question assessment', () => {
  ...
  for (const [index, question] of quiz.entries()) {
    assert.ok(question.id.startsWith('power-'), `question ${index + 1} should use the power id prefix`);
...
test('power sample size assessment covers learning points in the right order', () => {
  ...
  const orderedMilestones = [
    ['planning purpose', ['smallest useful effect']],
    ['minimum detectable effect', ['minimum detectable effect']],
    ['alpha false positives', ['false positive']],
    ['power detects real effects', ['statistical power']],
    ['type two and false negatives', ['false negative']],
    ['variance and noise', ['outcome variance']],
    ['square root sample scaling', ['square root']],
    ['underpowered interpretation', ['underpowered experiment']],
    ['effect over standard error', ['effect size divided by standard error']],
    ['smaller MDE costs sample', ['mde is made smaller']],
    ['cluster dependence', ['clustered users']],
    ['protocol', ['power planning protocol']],
    ['scenario application', ['team halves the mde']],
    ['tricky false claims', ['claim about a non-significant underpowered test is false']],
...
test('power sample size assessment avoids unsafe misconception keying', () => {
  ...
  const unsafePatterns = [
    ...
    /proof that .*no/i,
    /proves/i,
    /always increases power/i,
    /always means/i,
    /variance is irrelevant/i,
    /always be counted/i,
    /worth launching/i,
    /preserves the fixed-horizon design/i,
  ];
