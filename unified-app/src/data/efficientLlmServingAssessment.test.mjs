...
test('efficient LLM serving assessment avoids duplicate prompts and correct answers', () => {
  const prompts = EFFICIENT_LLM_SERVING_QUIZ.map((item) => normalize(item.prompt));
  const correctAnswers = EFFICIENT_LLM_SERVING_QUIZ.map((item) => normalize(correctAnswer(item)));
