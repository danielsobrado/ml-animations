...
test('frontier moe systems assessment has 100 production-ready questions', () => {
  ...
  for (const [index, item] of quiz.entries()) {
    assert.match(item.id, /^fmoe-\d{3}-[a-z0-9-]+$/);
    assert.equal(item.id.startsWith(`fmoe-${String(index + 1).padStart(3, '0')}`), true, `${item.id} is out of order`);
...
test('frontier moe systems assessment progresses through system learning points', () => {
  const { quiz } = getLessonAssessment('frontier-moe-systems');
  const ranges = [
    ['Foundation', 0, 20],
    ['Mechanism', 20, 50],
  ...
  const milestones = [
    ['frontier scope', ['frontier MoE systems lesson']],
    ['active accounting', ['Active FLOPs KPI']],
    ['all-to-all', ['All-to-All Dispatch']],
    ['shared experts', ['shared experts in this lesson']],
    ['failure modes', ['failure modes does the debugger include']],
    ['MoE equation', ['MoE(x)']],
    ['router logits', ['router logits described']],
    ['load loss', ['auxiliary load-balancing loss']],
    ['capacity overflow', ['capacity factor connect']],
    ['parallel cycle', ['distributed execution cycle']],
    ['distillation metric', ['router KL track']],
    ['application audit', ['paper says 671B total']],
    ['misconception traps', ['statement is false']],
    ['interview synthesis', ['production-ready frontier MoE takeaway']],
  ];
...
test('frontier moe systems assessment marks misconceptions as traps after setup', () => {
  const { quiz } = getLessonAssessment('frontier-moe-systems');
  const misconceptionTerms = [
    /removes serving complexity/i,
    /same as total stored parameters/i,
    /Shared experts are free/i,
    /compute on every token whether selected/i,
    /Load balancing can be ignored/i,
    /Capacity factor has no relationship/i,
    /avoids all cross-GPU token movement/i,
    /only fix is increasing total parameter count/i,
    /guaranteed just by naming experts/i,
    /Matching final answers alone guarantees/i,
    /permanently random/i,
    /remove the need to ask/i,
    /low active FLOPs percentage proves tail latency/i,
    /Dead experts mean every expert is overloaded/i,
    /solved once top-k routing works/i,
  ];
  const trapPrompt = /false|rejected|unsafe|wrong|dangerous|misleading|contradicts/i;
...
test('frontier moe systems assessment distributes correct answer positions per page', () => {
  const { quiz } = getLessonAssessment('frontier-moe-systems');
  const pageSize = 10;
