...
test('eigenvalue assessment has 100 curated questions', () => {
  const { quiz, labs } = getLessonAssessment('eigenvalue');
  ...
  for (const [index, item] of quiz.entries()) {
    assert.match(item.id, /^eig-\d{3}-[a-z0-9-]+$/);
    assert.equal(item.id.startsWith(`eig-${String(index + 1).padStart(3, '0')}`), true, `${item.id} is out of order`);
...
test('eigenvalue assessment avoids duplicate prompts and correct answers', () => {
...
test('eigenvalue assessment progresses through lesson learning points', () => {
  const { quiz } = getLessonAssessment('eigenvalue');
  const ranges = [
    ['Foundation', 0, 20],
    ['Mechanism', 20, 50],
  ...
  const milestones = [
    ['purpose', ['main purpose of the eigenvalue lesson']],
    ['tutorial', ['matrix A do to a vector v']],
    ['equation', ['core eigenvalue equation']],
    ['geometric example', ['unit circle']],
    ['practice panel', ['Practice panel use']],
    ['Rayleigh mechanism', ['Rayleigh quotient estimate']],
    ['application', ['rayleighNumerator([1,0], [3,0])']],
    ['misconception traps', ['Which eigenvalue claim is false']],
...
test('eigenvalue keeps misconception traps after setup', () => {
  ...
  const misconceptionTerms = [
    ...
    /ordinary eigenvectors of a general rectangular matrix/i,
    /Every square matrix has enough eigenvectors/i,
    /arbitrary non-orthogonal eigenvectors by necessity/i,
    /Any length ratio \|\|Av\|\| \/ \|\|v\|\| is automatically an eigenvalue/i,
    /Return v dot Av without dividing/i,
    /always converges no matter the spectrum/i,
    /negative must represent different axes/i,
    /Singular values are always identical/i,
    /full set of real eigenvectors/i,
    /off-diagonal zeros only/i,
    /automatically means most predictive feature/i,
    /Use determinant alone to recover every eigenvalue/i,
...
test('eigenvalue assessment avoids visible-page answer leakage', () => {
  const { quiz } = getLessonAssessment('eigenvalue');
  const pageSize = 10;
...
test('eigenvalue assessment distributes correct answer positions per page', () => {
  const { quiz } = getLessonAssessment('eigenvalue');
  const pageSize = 10;
