...
const LeakyReluGraphPanel = lazy(() => import('./LeakyReluGraphPanel'));
const PracticePanel = lazy(() => import('./PracticePanel'));

// Tab configuration
const tabs = [
    { id: 'animation', label: '1. Animation', icon: Play, color: 'from-purple-500 to-pink-500' },
    { id: 'graph', label: '2. Leaky ReLU Graph', icon: LineChart, color: 'from-green-500 to-emerald-500' },
...
export default function LeakyReluAnimation() {
    ...
    const renderPanel = () => {
        switch (activeTab) {
            ...
            case 'graph':
                return <Suspense fallback={<LoadingPanel />}><LeakyReluGraphPanel /></Suspense>;
            case 'practice':
