    ...
    <div className="space-y-6">
      ...
      <div className="bg-black/30 rounded-2xl p-6 border border-white/10 overflow-x-auto">
        <div className="min-w-[700px]">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="w-24 text-right text-sm text-gray-800">Tokens:</div>
          ...
          <div className={`flex items-center gap-4 mb-4 transition-all duration-500 ${currentStep >= 0 ? 'opacity-100' : 'opacity-30'}`}>
            <div className="w-24 text-right text-sm text-blue-600">Token Emb:</div>
            <div className="flex gap-4">
              {tokens.map((token, i) => (
                <div key={i} className="w-24">
                  <div className="bg-blue-900/30 border border-blue-500/30 rounded-lg p-2 h-16 flex flex-col justify-center">
                    ...
                    {showDimensions && <p className="text-xs text-center text-gray-700 mt-1">768-d</p>}
                  </div>
                </div>
              ))}
            </div>
            <div className="text-xs text-gray-700">E<sub>token</sub></div>
          </div>
          ...
          <div className={`flex items-center gap-4 mb-4 transition-all duration-500 ${currentStep >= 1 ? 'opacity-100' : 'opacity-30'}`}>
            ...
            <div className="text-xs text-gray-700">E<sub>segment</sub></div>
          </div>
          ...
          <div className={`flex items-center gap-4 mb-4 transition-all duration-500 ${currentStep >= 2 ? 'opacity-100' : 'opacity-30'}`}>
            <div className="w-24 text-right text-sm text-purple-600">Position Emb:</div>
            <div className="flex gap-4">
              {tokens.map((token, i) => (
                <div key={i} className="w-24">
                  <div className="bg-purple-900/30 border border-purple-500/30 rounded-lg p-2 h-16 flex flex-col justify-center">
                    ...
                    <p className="text-xs text-center text-purple-600 mt-1">pos={i}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="text-xs text-gray-700">E<sub>position</sub></div>
          </div>
          ...
          <div className={`flex items-center gap-4 transition-all duration-500 ${currentStep >= 3 ? 'opacity-100' : 'opacity-30'}`}>
            <div className="w-24 text-right text-sm text-orange-600 font-bold">Input Emb:</div>
            <div className="flex gap-4">
              {tokens.map((token, i) => (
                <div key={i} className="w-24">
            ...
            <div className="text-xs text-orange-600 font-bold">→ to Encoder</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/30 rounded-xl p-4 border border-white/10 text-center">
        <p className="font-mono text-lg">
          <span className="text-orange-600">E<sub>input</sub></span> =
          <span className="text-blue-600"> E<sub>token</sub></span> +
          <span className="text-green-400"> E<sub>segment</sub></span> +
          <span className="text-purple-600"> E<sub>position</sub></span>
        </p>
        <p className="text-sm text-gray-700 mt-2">All embeddings are 768-dimensional vectors (for BERT-Base)</p>
      </div>

      {/* Detailed Explanations */}
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bg-blue-900/20 rounded-xl p-4 border border-blue-500/30">
          <h4 className="font-bold text-blue-600 mb-2">🔤 Token Embeddings</h4>
        ...
        <div className="bg-purple-900/20 rounded-xl p-4 border border-purple-500/30">
          <h4 className="font-bold text-purple-600 mb-2">📍 Position Embeddings</h4>
          <ul className="text-sm text-gray-700 space-y-2">
          ...
          <div className="flex gap-2 min-w-max items-end">
            {['[CLS]', 'The', 'cat', 'sat', '[SEP]', 'It', 'was', 'happy', '[SEP]'].map((token, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
                ...
                <span className="text-xs text-purple-600">pos {i}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-black/40 rounded-xl p-4 border border-white/10">
        <p className="text-sm text-gray-800 mb-3">🐍 PyTorch Implementation:</p>
