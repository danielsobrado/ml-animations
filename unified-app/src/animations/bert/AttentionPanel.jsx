  ...
  const AttentionMatrix = ({ pattern, title, size = 'large' }) => {
    ...
    return (
      <div className="inline-block">
        {title && <p className="text-xs text-gray-800 mb-1 text-center">{title}</p>}
        <div className="flex flex-col">
          <div className="flex">
            <div className={`${cellSize}`} />
            {tokens.map((t, i) => (
              <div key={i} className={`${cellSize} ${textSize} flex items-center justify-center text-gray-700 font-mono`}>
                {t.slice(0, 3)}
              </div>
            ))}
          </div>
          {tokens.map((rowToken, i) => (
            <div key={i} className="flex">
              <div className={`${cellSize} ${textSize} flex items-center justify-center text-gray-700 font-mono`}>
                {rowToken.slice(0, 3)}
              </div>
              {tokens.map((_, j) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-black/30 rounded-2xl p-6 border border-white/10 overflow-x-auto">
        ...
        <div className="flex items-center justify-center gap-4 mb-8 min-w-max">
          ...
          <div className={`text-center transition-all duration-500 ${currentStep >= 0 ? 'opacity-100' : 'opacity-30'}`}>
            <div className="bg-blue-600/30 border border-blue-500 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-800">[6, 768]</p>
            </div>
          </div>
          ...
          <div className={`text-center transition-all duration-500 ${currentStep >= 1 ? 'opacity-100' : 'opacity-30'}`}>
            ...
            <p className="text-xs text-gray-800 mt-1">[6, 12, 64]</p>
          </div>
          <span className="text-gray-700">→</span>
          ...
          <div className={`text-center transition-all duration-500 ${currentStep >= 2 ? 'opacity-100' : 'opacity-30'}`}>
            <div className="bg-orange-600/30 border border-orange-500 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-800">[6, 6]</p>
            </div>
          </div>
          <span className="text-gray-700">→</span>
          ...
          <div className={`text-center transition-all duration-500 ${currentStep >= 3 ? 'opacity-100' : 'opacity-30'}`}>
            <div className="bg-yellow-600/30 border border-yellow-500 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-800">weights</p>
            </div>
          </div>
          <span className="text-gray-700">→</span>
          ...
          <div className={`text-center transition-all duration-500 ${currentStep >= 4 ? 'opacity-100' : 'opacity-30'}`}>
            <div className="bg-teal-600/30 border border-teal-500 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-800">[6, 64]</p>
            </div>
          </div>
          <span className="text-gray-700">→</span>
        ...
        {currentStep >= 2 && (
          <div className="mt-6">
            {showAllHeads ? (
              <div>
                <h4 className="text-center text-gray-800 mb-4">All 12 Attention Heads (showing 4 representative patterns)</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 justify-items-center">
                  {headPatterns.map((head, i) => (
                    <div key={i} className="text-center">
                      ...
                      <p className="text-xs text-gray-700 mt-1">{head.description}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-black/30 rounded-xl p-4 border border-white/10">
          <h4 className="font-bold text-purple-600 mb-3">📊 BERT Attention Stats</h4>
          <table className="w-full text-sm">
            <tbody>
              <tr className="border-b border-white/10">
                <td className="py-2 text-gray-800">Number of heads</td>
                <td className="py-2 text-right font-mono">12 (Base) / 16 (Large)</td>
              </tr>
              <tr className="border-b border-white/10">
                <td className="py-2 text-gray-800">Head dimension (dₖ)</td>
                <td className="py-2 text-right font-mono">64</td>
              </tr>
              <tr className="border-b border-white/10">
                <td className="py-2 text-gray-800">Total dimension</td>
                <td className="py-2 text-right font-mono">768 (Base) / 1024 (Large)</td>
              </tr>
              <tr>
                <td className="py-2 text-gray-800">Attention layers</td>
      ...
      <div className="bg-gradient-to-r from-blue-900/30 to-green-900/30 rounded-xl p-4 border border-blue-500/30">
        <h4 className="font-bold text-blue-600 mb-3">🔄 Bidirectional Attention (BERT's Key Innovation)</h4>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <p className="text-sm font-medium text-gray-700 mb-2">GPT (Unidirectional):</p>
            ...
            <p className="text-xs text-gray-700 mt-2">Each token only sees tokens to its left</p>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-700 mb-2">BERT (Bidirectional):</p>
            <div className="flex gap-1">
              {['The', 'cat', 'sat', 'on'].map((t, i) => (
                <div key={i} className="relative">
      ...
      <div className="bg-black/40 rounded-xl p-4 border border-white/10">
        <p className="text-sm text-gray-800 mb-3">🐍 PyTorch Multi-Head Attention:</p>
