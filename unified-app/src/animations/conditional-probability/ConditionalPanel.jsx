    ...
    return (
        <div className="p-8 h-full flex flex-col items-center overflow-y-auto">
            <div className="max-w-3xl w-full text-center mb-8">
                <h2 className="text-3xl font-bold text-emerald-600 dark:text-emerald-400 mb-4">Conditional Probability</h2>
            ...
            <div className="bg-slate-800 p-6 rounded-xl border border-emerald-500/50 w-full max-w-4xl mb-8">
                ...
                <p className="text-slate-700 dark:text-center mb-6">
                    What's the probability of drawing a <strong className="text-yellow-400">King</strong>
                    {filterFaceCards && <span>, given it's a <strong className="text-cyan-600 dark:text-cyan-400">Face Card</strong>?</span>}
                    {!filterFaceCards && <span>?</span>}
                </p>

                {/* Toggle */}
                <div className="flex items-center justify-center gap-4 mb-6">
                    <span className="text-slate-800 dark:text-slate-400">Show all cards</span>
                    <button
                    ...
                    <span className="text-cyan-600 dark:text-cyan-400 font-bold">Filter: Face Cards only</span>
                </div>
                ...
                <div className="bg-slate-900 p-6 rounded-lg border border-slate-700">
                    <div className="grid md:grid-cols-3 gap-4 text-center">
                        <div>
                            <div className="text-slate-800 dark:text-sm mb-2">Sample Space</div>
                            <div className="text-3xl font-bold text-white">{totalCount}</div>
                            <div className="text-xs text-slate-700 dark:text-slate-500 mt-1">
                                {filterFaceCards ? 'Face cards' : 'All cards'}
                            </div>
                        </div>
                        <div>
                            <div className="text-slate-800 dark:text-sm mb-2">Kings</div>
