    ...
    return (
        <div className="p-8 h-full flex flex-col items-center overflow-y-auto">
            <div className="max-w-3xl w-full text-center mb-8">
                <h2 className="text-3xl font-bold text-pink-600 mb-4">Text Generation (Mini-LLM)</h2>
            ...
            <div className="grid md:grid-cols-2 gap-8 w-full max-w-6xl">
                ...
                <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 flex flex-col">
                    ...
                    <textarea
                        ...
                        className="w-full h-40 bg-slate-900 border border-slate-600 rounded-lg p-4 text-slate-700 focus:border-pink-500 outline-none resize-none"
                    ...
                    <div className="flex-1 overflow-y-auto font-mono text-sm space-y-2 pr-2">
                        {Object.entries(chain).map(([word, nextWords]) => (
                            <div key={word} className="bg-slate-900 p-2 rounded border border-slate-700">
                                <span className="text-pink-600 font-bold">{word}</span>
                                <span className="text-slate-700"> ➔ </span>
                                <span className="text-slate-700">[{nextWords.join(', ')}]</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="w-full max-w-6xl mt-8">
                ...
                {generatedText && (
                    <div className="mt-8 p-8 bg-slate-900 rounded-2xl border-2 border-pink-500/50 text-center">
                        <h3 className="text-slate-700 uppercase tracking-wider mb-4">Output</h3>
