...
import { BookOpen, Grid3X3, Calculator, GitCompare, Code, GraduationCap } from 'lucide-react';

// Lazy load panels
const IntroPanel = lazy(() => import('./IntroPanel'));
const CooccurrencePanel = lazy(() => import('./CooccurrencePanel'));
const ObjectivePanel = lazy(() => import('./ObjectivePanel'));
...
const tabs = [
    ...
    { id: 'cooccurrence', label: '2. Co-occurrence', icon: Grid3X3, color: 'from-green-500 to-emerald-500' },
    { id: 'objective', label: '3. GloVe Objective', icon: Calculator, color: 'from-purple-500 to-pink-500' },
    { id: 'comparison', label: '4. vs Word2Vec', icon: GitCompare, color: 'from-amber-500 to-orange-500' },
...
export default function GloveAnimation() {
    ...
    const renderPanel = () => {
        switch (activeTab) {
            ...
            case 'cooccurrence':
                return <Suspense fallback={<LoadingPanel />}><CooccurrencePanel /></Suspense>;
            case 'objective':
                return <Suspense fallback={<LoadingPanel />}><ObjectivePanel /></Suspense>;
            case 'comparison':
