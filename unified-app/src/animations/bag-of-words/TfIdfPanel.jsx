  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-black/30 rounded-2xl p-6 border border-white/10">
        ...
        {currentStep === 0 && (
          <div className="space-y-4">
            <h4 className="text-lg font-bold text-gray-700">Document Corpus (N = {data.N})</h4>
            {documents.map((doc, i) => (
              <div
                key={i}
              ...
              >
                <span className="text-blue-600 font-mono text-sm">Doc {i + 1}:</span>
        ...
        {currentStep === 1 && (
          <div className="space-y-4">
            <div className="flex items-center gap-4 mb-4">
              <h4 className="text-lg font-bold text-gray-700">Term Frequency (TF)</h4>
              <select
                value={selectedDoc}
            ...
            <div className="bg-blue-900/20 rounded-lg p-3 mb-4">
              <p className="text-sm text-blue-600">Formula: TF(t, d) = count(t in d) / |d|</p>
              <p className="text-xs text-gray-800 mt-1">Word count divided by total words in document</p>
            </div>

            <p className="text-sm text-gray-800 mb-2">"{documents[selectedDoc]}"</p>
        ...
        {currentStep === 2 && (
          <div className="space-y-4">
            <h4 className="text-lg font-bold text-gray-700">Document Frequency (DF)</h4>
            <div className="bg-purple-900/20 rounded-lg p-3 mb-4">
              <p className="text-sm text-purple-600">DF(t) = number of documents containing term t</p>
            </div>
        ...
        {currentStep === 3 && (
          <div className="space-y-4">
            <h4 className="text-lg font-bold text-gray-700">Inverse Document Frequency (IDF)</h4>
            <div className="bg-orange-900/20 rounded-lg p-3 mb-4">
              <p className="text-sm text-orange-600">IDF(t) = log(N / DF(t))</p>
            ...
            <div className="bg-gray-800 rounded-lg p-4 mt-4">
              <p className="text-sm text-gray-800 mb-2">Notice:</p>
        ...
        {currentStep === 4 && (
          <div className="space-y-4">
            <div className="flex items-center gap-4 mb-4">
              <h4 className="text-lg font-bold text-gray-700">TF-IDF Scores</h4>
              <select
                value={selectedDoc}
                onChange={(e) => setSelectedDoc(parseInt(e.target.value))}
                className="bg-gray-700 border border-gray-600 rounded px-3 py-1"
            ...
            <div className="mt-6 overflow-x-auto">
              <h5 className="text-sm text-gray-800 mb-2">Complete TF-IDF Matrix:</h5>
              <table className="w-full text-sm">
                <thead>
                  <tr>
                    <th className="px-2 py-1 text-left text-gray-800">Doc</th>
                    {data.vocab.map((word, i) => (
                ...
                <tbody>
                  {documents.map((_, docIdx) => (
                    <tr key={docIdx} className={docIdx === selectedDoc ? 'bg-yellow-900/20' : ''}>
                      <td className="px-2 py-1 text-gray-800">Doc {docIdx + 1}</td>
                      {data.vocab.map((word, i) => (
      ...
      <div className="bg-gradient-to-r from-yellow-900/20 to-orange-900/20 rounded-xl p-6 border border-yellow-500/30">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="bg-black/30 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-800 mt-2">Normalized word count in document</p>
          </div>
          <div className="bg-black/30 rounded-lg p-4">
            <h5 className="text-purple-600 font-medium mb-2">Inverse Doc Frequency</h5>
            <div className="bg-black/50 rounded p-2 font-mono text-sm text-center">
              IDF(t) = log(N / DF(t))
            </div>
            <p className="text-xs text-gray-800 mt-2">Penalizes common words</p>
          </div>
          <div className="bg-black/30 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-800 mt-2">Final importance weight</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/40 rounded-xl p-4 border border-white/10">
        <p className="text-sm text-gray-800 mb-3">🐍 Python with scikit-learn:</p>
