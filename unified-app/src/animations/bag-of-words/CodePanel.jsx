...
export default function CodePanel() {
  ...
  const codeExamples = {
    sklearn: {
      title: 'scikit-learn',
      description: 'Most common library for traditional ML in Python',
      codes: [
        {
          title: 'Bag of Words with CountVectorizer',
...
print("\\nWord to Index:", vectorizer.vocabulary_)`
        },
        {
          title: 'TF-IDF with TfidfVectorizer',
...
print("\\nIDF values:", {k: round(v, 3) for k, v in idf_values.items()})`
        },
        {
...
print(f"Prediction: {'Positive' if pipeline.predict(new_text)[0] else 'Negative'}")`
        },
        {
...
for idx, score in find_similar(0):
    print(f"  Doc {idx} (similarity: {score:.3f}): {documents[idx][:50]}...")`
        }
      ]
    },
    numpy: {
      title: 'NumPy (From Scratch)',
      description: 'Build BoW and TF-IDF from scratch',
      codes: [
        {
...
# Find most similar documents
print("\\nDoc 0 is most similar to:", np.argmax(sim_matrix[0, 1:]) + 1)`
        }
      ]
    },
    nltk: {
      title: 'NLTK',
      description: 'Natural Language Toolkit for advanced preprocessing',
      codes: [
        {
...
print("After lemmatization:", lemmatized)`
        ...
        {
          title: 'NLTK with sklearn Pipeline',
          code: `import nltk
from nltk.tokenize import word_tokenize
from nltk.corpus import stopwords
