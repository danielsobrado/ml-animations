    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold mb-2">
          <span className="text-blue-600">Word2Vec</span>: Words in Vector Space
        </h2>
      ...
      <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-2xl p-6 border border-blue-500/30">
        ...
        <div className="text-center text-xl text-gray-700 italic">
          "You shall know a word by the company it keeps"
        </div>
        <p className="text-center text-sm text-gray-700 mt-2">— J.R. Firth (1957)</p>
        ...
        <div className="bg-black/30 rounded-xl p-6 border border-white/10">
          ...
          <div className="space-y-4">
            <div className="bg-green-900/30 rounded-lg p-4 border border-green-500/30">
              ...
              <div className="text-xs text-gray-700 mt-2">Better for rare words</div>
            </div>
            <div className="bg-yellow-900/30 rounded-lg p-4 border border-yellow-500/30">
              ...
              <div className="text-xs text-gray-700 mt-2">Faster training, good for frequent words</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/30 rounded-xl p-6 border border-white/10">
        ...
        <table className="w-full">
          <thead>
            <tr>
              <th className="text-left p-2 text-gray-800">Aspect</th>
          ...
          <tbody>
            {traditionalVsWord2Vec.map((row, i) => (
              <tr key={i} className="border-t border-white/10">
                <td className="p-2 text-gray-700">{row.aspect}</td>
                <td className="p-2 text-sm text-gray-800">{row.traditional}</td>
                <td className="p-2 text-sm text-gray-800">{row.word2vec}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          {famousAnalogies.map((analogy, i) => (
            <div key={i} className="bg-black/30 rounded-lg p-4 animate-fadeIn" style={{ animationDelay: `${i * 100}ms` }}>
              <div className="flex flex-wrap items-center gap-2 font-mono text-lg">
                <span className="text-blue-600">{analogy.a}</span>
              ...
              <p className="text-xs text-gray-700 mt-2">{analogy.explanation}</p>
            </div>
          ))}
        </div>
      ...
      <div className="bg-black/30 rounded-xl p-6 border border-white/10">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 font-mono text-sm">
              <div className="flex items-center gap-2">
                <span className="text-gray-800 w-12">cat:</span>
                <span className="text-red-400">[1, 0, 0, 0, 0, ...]</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-gray-800 w-12">dog:</span>
                <span className="text-red-400">[0, 1, 0, 0, 0, ...]</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-gray-800 w-12">kitten:</span>
                <span className="text-red-400">[0, 0, 1, 0, 0, ...]</span>
              </div>
            </div>
            <p className="text-xs text-gray-700">All words equally different (similarity = 0)</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-700">Similar words have similar vectors!</p>
          </div>
        </div>
      </div>
