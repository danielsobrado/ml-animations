...
import { BookOpen, Target, Shuffle, Zap, Brain, Code, GraduationCap } from 'lucide-react';

// Lazy load panels
const IntroPanel = lazy(() => import('./IntroPanel'));
const SkipGramPanel = lazy(() => import('./SkipGramPanel'));
const CbowPanel = lazy(() => import('./CbowPanel'));
const NegativeSamplingPanel = lazy(() => import('./NegativeSamplingPanel'));
...
const tabs = [
    ...
    { id: 'skipgram', label: '2. Skip-gram', icon: Target, color: 'from-green-500 to-emerald-500' },
    { id: 'cbow', label: '3. CBOW', icon: Shuffle, color: 'from-amber-500 to-yellow-500' },
    { id: 'negative', label: '4. Negative Sampling', icon: Zap, color: 'from-orange-500 to-red-500' },
    { id: 'embeddings', label: '5. Embeddings', icon: Brain, color: 'from-purple-500 to-pink-500' },
...
export default function Word2VecAnimation() {
    ...
    const renderPanel = () => {
        switch (activeTab) {
            ...
            case 'skipgram':
                return <Suspense fallback={<LoadingPanel />}><SkipGramPanel /></Suspense>;
            case 'cbow':
                return <Suspense fallback={<LoadingPanel />}><CbowPanel /></Suspense>;
            case 'negative':
                return <Suspense fallback={<LoadingPanel />}><NegativeSamplingPanel /></Suspense>;
            case 'embeddings':
