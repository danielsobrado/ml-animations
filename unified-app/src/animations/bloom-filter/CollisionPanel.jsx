    ...
    return (
        <div className="p-8 h-full flex flex-col items-center">
            ...
            <div className="flex gap-2 mb-8">
                {bits.map((bit, i) => (
                    <div key={i} className={`w-12 h-12 rounded border-2 flex items-center justify-center font-bold text-xl transition-colors ${bit === 1 ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
                        {bit}
                    </div>
                ))}
            </div>
