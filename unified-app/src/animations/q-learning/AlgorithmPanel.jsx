        ...
        <div className="p-8 h-full flex flex-col items-center overflow-y-auto">
            <div className="max-w-3xl w-full text-center mb-8">
                ...
                <div className="bg-slate-800 p-4 rounded-lg font-mono text-sm inline-block shadow-lg border border-slate-700">
                    Q(s,a) ← Q(s,a) + <span className="text-purple-600">α</span> [
                    <span className="text-green-400">R</span> +
                    <span className="text-orange-600">γ</span> max Q(s',a') - Q(s,a) ]
                </div>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-12 w-full max-w-6xl">
                ...
                <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 space-y-6">
                    ...
                    <div>
                        <label className="text-sm text-slate-800 block mb-1">Old Q-Value (Q(s,a))</label>
                    ...
                    <div>
                        <label className="text-sm text-blue-600 block mb-1">Max Future Q (max Q(s',a'))</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-sm text-purple-600 block mb-1">Learning Rate (α)</label>
                        ...
                        <div>
                            <label className="text-sm text-orange-600 block mb-1">Discount (γ)</label>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 relative overflow-hidden">
                        ...
                        <h4 className="text-sm text-slate-800 uppercase tracking-wider mb-2">1. The Target (Reality + Promise)</h4>
                        <div className="font-mono text-xl text-white">
                            {reward} + {gamma} × {qNextMax} = <span className="text-blue-600 font-bold">{target.toFixed(2)}</span>
                        </div>
                        <p className="text-xs text-slate-700 mt-2">Reward + Discounted Future Value</p>
                    </div>

                    {/* Step 2: Error */}
                    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1 h-full bg-red-500"></div>
                        <h4 className="text-sm text-slate-800 uppercase tracking-wider mb-2">2. The Surprise (TD Error)</h4>
                        <div className="font-mono text-xl text-white">
                            {target.toFixed(2)} - {qOld} = <span className="text-red-400 font-bold">{tdError.toFixed(2)}</span>
                        </div>
                        <p className="text-xs text-slate-700 mt-2">Target - Old Expectation</p>
                    </div>

                    {/* Step 3: Update */}
                    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1 h-full bg-green-500"></div>
                        <h4 className="text-sm text-slate-800 uppercase tracking-wider mb-2">3. The New Value</h4>
                        <div className="font-mono text-xl text-white">
                            {qOld} + {alpha} × {tdError.toFixed(2)} = <span className="text-green-400 font-bold text-3xl">{qNew.toFixed(2)}</span>
                        </div>
                        <p className="text-xs text-slate-700 mt-2">Nudging the old value towards the target</p>
                    </div>
                </div>
            </div>
        </div>
